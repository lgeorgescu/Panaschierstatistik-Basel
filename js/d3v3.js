d3v3 = function() {
  ...
  var d3v3_document = document, d3v3_window = window;
  try {
  ...
  d3v3_class(d3v3_Set, {
    has: function(value) {
      return d3v3_map_prefix + value in this;
    },
    add: function(value) {
      this[d3v3_map_prefix + value] = true;
      return value;
    },
    remove: function(value) {
      value = d3v3_map_prefix + value;
    ...
    forEach: function(f) {
      for (var value in this) {
        if (value.charCodeAt(0) === d3v3_map_prefixCode) {
  ...
  function d3v3_eventSuppress(target, type) {
    function off() {
      target.on(type, null);
    }
  ...
  d3v3.behavior.drag = function() {
    ...
    function mousedown() {
      ...
      function dragend() {
        ...
        if (moved) {
          d3v3_eventCancel();
          if (d3v3.event.target === eventTarget) d3v3_eventSuppress(w, "click");
        }
  ...
  d3v3_selectionPrototype.insert = function(name, before) {
    name = d3v3.ns.qualify(name);
    if (typeof before !== "function") before = d3v3_selection_selector(before);
    function insert(d, i) {
      return this.insertBefore(d3v3_document.createElementNS(this.namespaceURI, name), before.call(this, d, i));
    }
    function insertNS(d, i) {
      return this.insertBefore(d3v3_document.createElementNS(name.space, name.local), before.call(this, d, i));
    }
  ...
  d3v3.select = function(node) {
    ...
    group.parentNode = d3v3_selectRoot;
  ...
  var d3v3_selectionRoot = d3v3.select(d3v3_selectRoot);
  d3v3.behavior.zoom = function() {
    ...
    function mousedown() {
      ...
      function mouseup() {
        ...
        if (moved && d3v3.event.target === eventTarget) d3v3_eventSuppress(w, "click.zoom");
      }
    }
    function mousewheel() {
  ...
  var π = Math.PI, ε = 1e-6, d3v3_radians = π / 180, d3v3_degrees = 180 / π;
  function d3v3_sgn(x) {
  ...
  function d3v3_lab_hcl(l, a, b) {
    return l > 0 ? d3v3_hcl(Math.atan2(b, a) * d3v3_degrees, Math.sqrt(a * a + b * b), l) : d3v3_hcl(NaN, NaN, l);
  }
  ...
  var d3v3_geo_areaSum, d3v3_geo_areaRingSum;
  var d3v3_geo_area = {
    sphere: function() {
  ...
  function d3v3_geo_areaRingStart() {
    ...
    function nextPoint(λ, φ) {
      ...
      d3v3_geo_areaRingSum += Math.atan2(v, u);
  ...
  d3v3.geo.bounds = function() {
    ...
    var bound = {
      ...
      polygonEnd: function() {
        ...
        if (d3v3_geo_areaRingSum < 0) λ0 = -(λ1 = 180), φ0 = -(φ1 = 90); else if (dλSum > ε) φ1 = 90; else if (dλSum < -ε) φ0 = -90;
    ...
    function linePoint(λ, φ) {
      var p = d3v3_geo_cartesian([ λ * d3v3_radians, φ * d3v3_radians ]);
      if (p0) {
        ...
        var dλ = λ - λ_, s = dλ > 0 ? 1 : -1, λi = inflection[0] * d3v3_degrees * s, antimeridian = Math.abs(dλ) > 180;
        if (antimeridian ^ (s * λ_ < λi && λi < s * λ)) {
          var φi = inflection[1] * d3v3_degrees;
          if (φi > φ1) φ1 = φi;
  ...
  function d3v3_geo_clipPolygon(segments, compare, inside, interpolate, listener) {
    var subject = [], clip = [];
    ...
    return function(listener) {
      ...
      var clip = {
        ...
        polygonEnd: function() {
          ...
          if (segments.length) {
            d3v3_geo_clipPolygon(segments, d3v3_geo_clipSort, null, interpolate, listener);
  ...
  function d3v3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, notHemisphere = Math.abs(cr) > ε, interpolate = d3v3_geo_circleInterpolate(radius, 6 * d3v3_radians);
  ...
  var d3v3_geo_clipViewMAX = 1e9;
  function d3v3_geo_clipView(x0, y0, x1, y1) {
    return function(listener) {
      var listener_ = listener, bufferListener = d3v3_geo_clipBufferListener(), segments, polygon, ring;
      var clip = {
        point: point,
        lineStart: lineStart,
        lineEnd: lineEnd,
        ...
        polygonEnd: function() {
          ...
          if ((segments = d3v3.merge(segments)).length) {
            listener.polygonStart();
            d3v3_geo_clipPolygon(segments, compare, inside, interpolate, listener);
            listener.polygonEnd();
      ...
      function linePoint(x, y) {
        x = Math.max(-d3v3_geo_clipViewMAX, Math.min(d3v3_geo_clipViewMAX, x));
        y = Math.max(-d3v3_geo_clipViewMAX, Math.min(d3v3_geo_clipViewMAX, y));
        var v = visible(x, y);
    ...
    function clipLine(a, b) {
      ...
      if (d3v3_geo_clipViewT(x0 - a[0], dx, t) && d3v3_geo_clipViewT(a[0] - x1, -dx, t) && d3v3_geo_clipViewT(y0 - a[1], dy, t) && d3v3_geo_clipViewT(a[1] - y1, -dy, t)) {
        if (t[1] < 1) {
          b[0] = a[0] + t[1] * dx;
          b[1] = a[1] + t[1] * dy;
        }
  ...
  function d3v3_geo_clipViewT(num, denominator, t) {
  ...
  function d3v3_geo_projectionMutator(projectAt) {
    ...
    }), k = 150, x = 480, y = 250, λ = 0, φ = 0, δλ = 0, δφ = 0, δγ = 0, δx, δy, preclip = d3v3_geo_clipAntimeridian, postclip = d3v3_identity, clipAngle = null, clipExtent = null;
    ...
    projection.stream = function(stream) {
      return d3v3_geo_projectionRadiansRotate(rotate, preclip(projectResample(postclip(stream))));
    };
    ...
    projection.clipExtent = function(_) {
      ...
      clipExtent = _;
      postclip = _ == null ? d3v3_identity : d3v3_geo_clipView(_[0][0], _[0][1], _[1][0], _[1][1]);
      return projection;
    };
  ...
  d3v3.geo.distance = function(a, b) {
    var Δλ = (b[0] - a[0]) * d3v3_radians, φ0 = a[1] * d3v3_radians, φ1 = b[1] * d3v3_radians, sinΔλ = Math.sin(Δλ), cosΔλ = Math.cos(Δλ), sinφ0 = Math.sin(φ0), cosφ0 = Math.cos(φ0), sinφ1 = Math.sin(φ1), cosφ1 = Math.cos(φ1), t;
  ...
  function d3v3_geo_lengthLineStart() {
    var λ0, sinφ0, cosφ0;
    d3v3_geo_length.point = function(λ, φ) {
      λ0 = λ * d3v3_radians, sinφ0 = Math.sin(φ *= d3v3_radians), cosφ0 = Math.cos(φ);
    ...
    function nextPoint(λ, φ) {
      var sinφ = Math.sin(φ *= d3v3_radians), cosφ = Math.cos(φ), t = Math.abs((λ *= d3v3_radians) - λ0), cosΔλ = Math.cos(t);
      d3v3_geo_lengthSum += Math.atan2(Math.sqrt((t = cosφ * Math.sin(t)) * t + (t = cosφ0 * sinφ - sinφ0 * cosφ * cosΔλ) * t), sinφ0 * sinφ + cosφ0 * cosφ * cosΔλ);
      λ0 = λ, sinφ0 = sinφ, cosφ0 = cosφ;
    }
  }
  function d3v3_geo_conic(projectAt) {
    var φ0 = 0, φ1 = π / 3, m = d3v3_geo_projectionMutator(projectAt), p = m(φ0, φ1);
  ...
  function d3v3_geo_conicEqualArea(φ0, φ1) {
    ...
    forward.invert = function(x, y) {
      var ρ0_y = ρ0 - y;
      return [ Math.atan2(x, ρ0_y) / n, d3v3_asin((C - (x * x + ρ0_y * ρ0_y) * n * n) / (2 * n)) ];
    };
    return forward;
  }
  ...
  function d3v3_geo_conicConformal(φ0, φ1) {
    ...
    forward.invert = function(x, y) {
      var ρ0_y = F - y, ρ = d3v3_sgn(n) * Math.sqrt(x * x + ρ0_y * ρ0_y);
  ...
  function d3v3_geo_conicEquidistant(φ0, φ1) {
    var cosφ0 = Math.cos(φ0), n = φ0 === φ1 ? Math.sin(φ0) : (cosφ0 - Math.cos(φ1)) / (φ1 - φ0), G = cosφ0 / n + φ0;
    if (Math.abs(n) < ε) return d3v3_geo_equirectangular;
    function forward(λ, φ) {
      var ρ = G - φ;
    ...
    forward.invert = function(x, y) {
      var ρ0_y = G - y;
      return [ Math.atan2(x, ρ0_y) / n, G - d3v3_sgn(n) * Math.sqrt(x * x + ρ0_y * ρ0_y) ];
    };
    return forward;
  }
  ...
  function d3v3_geo_transverseMercator(λ, φ) {
    var B = Math.cos(φ) * Math.sin(λ);
    return [ Math.log((1 + B) / (1 - B)) / 2, Math.atan2(Math.tan(φ), Math.cos(λ)) ];
  }
  d3v3_geo_transverseMercator.invert = function(x, y) {
    return [ Math.atan2(d3v3_sinh(x), Math.cos(y)), d3v3_asin(Math.sin(y) / d3v3_cosh(x)) ];
  };
  (d3v3.geo.transverseMercator = function() {
    return d3v3_geo_mercatorProjection(d3v3_geo_transverseMercator);
  ...
  d3v3.geom.hull = function(vertices) {
    var x = d3v3_svg_lineX, y = d3v3_svg_lineY;
    if (arguments.length) return hull(vertices);
    function hull(data) {
      if (data.length < 3) return [];
      var fx = d3v3_functor(x), fy = d3v3_functor(y), n = data.length, vertices, plen = n - 1, points = [], stack = [], d, i, j, h = 0, x1, y1, x2, y2, u, v, a, sp;
      if (fx === d3v3_svg_lineX && y === d3v3_svg_lineY) vertices = data; else for (i = 0, 
      vertices = []; i < n; ++i) {
  ...
  d3v3.geom.delaunay = function(vertices) {
    ...
    d3v3_geom_voronoiTessellate(vertices, function(e) {
  ...
  d3v3.geom.voronoi = function(points) {
    var size = null, x = d3v3_svg_lineX, y = d3v3_svg_lineY, clip;
    if (arguments.length) return voronoi(points);
    function voronoi(data) {
      var points, polygons = data.map(function() {
        return [];
      }), fx = d3v3_functor(x), fy = d3v3_functor(y), d, i, n = data.length, Z = 1e6;
      if (fx === d3v3_svg_lineX && fy === d3v3_svg_lineY) points = data; else for (points = [], 
      i = 0; i < n; ++i) {
        points.push([ +fx.call(this, d = data[i], i), +fy.call(this, d, i) ]);
      }
      d3v3_geom_voronoiTessellate(points, function(e) {
    ...
    voronoi.size = function(_) {
      ...
      if (_ == null) {
        ...
        size = [ +_[0], +_[1] ];
        clip = d3v3.geom.polygon([ [ 0, 0 ], [ 0, size[1] ], size, [ size[0], 0 ] ]).clip;
      }
      return voronoi;
    };
    voronoi.links = function(data) {
      var points, graph = data.map(function() {
        return [];
      }), links = [], fx = d3v3_functor(x), fy = d3v3_functor(y), d, i, n = data.length;
      if (fx === d3v3_svg_lineX && fy === d3v3_svg_lineY) points = data; else for (i = 0; i < n; ++i) {
        points.push([ +fx.call(this, d = data[i], i), +fy.call(this, d, i) ]);
      }
      d3v3_geom_voronoiTessellate(points, function(e) {
    ...
    voronoi.triangles = function(data) {
      if (x === d3v3_svg_lineX && y === d3v3_svg_lineY) return d3v3.geom.delaunay(data);
      var points, point, fx = d3v3_functor(x), fy = d3v3_functor(y), d, i, n;
  ...
  var d3v3_geom_voronoiOpposite = {
    l: "r",
    r: "l"
  };
  function d3v3_geom_voronoiTessellate(points, callback) {
    var Sites = {
    ...
    var EdgeList = {
      ...
      rightRegion: function(he) {
        return he.edge == null ? Sites.bottomSite : he.edge.region[d3v3_geom_voronoiOpposite[he.side]];
      }
    };
    var Geom = {
      ...
      endPoint: function(edge, side, site) {
        edge.ep[side] = site;
        if (!edge.ep[d3v3_geom_voronoiOpposite[side]]) return;
        callback(edge);
      },
    ...
    while (true) {
      ...
      if (newSite && (EventQueue.empty() || newSite.y < newIntStar.y || newSite.y == newIntStar.y && newSite.x < newIntStar.x)) {
        ...
        Geom.endPoint(e, d3v3_geom_voronoiOpposite[pm], v);
  ...
  d3v3.geom.quadtree = function(points, x1, y1, x2, y2) {
    var x = d3v3_svg_lineX, y = d3v3_svg_lineY, compat;
  ...
  function d3v3_interpolateString(a, b) {
    ...
    for (i = 0, n = q.length; (m = d3v3_interpolate_number.exec(a)) && i < n; ++i) {
      ...
      if (o.x == m[0]) {
        ...
        o.x = d3v3_interpolateNumber(parseFloat(m[0]), parseFloat(o.x));
      }
    }
    while (i < n) {
      o = q.pop();
  ...
  function d3v3_interpolateByName(name) {
    return name == "transform" ? d3v3_interpolateTransform : d3v3_interpolate;
  }
  d3v3.interpolators = [ function(a, b) {
    var t = typeof b;
    return (t === "string" ? d3v3_rgb_names.has(b) || /^(#|rgb\(|hsl\()/.test(b) ? d3v3_interpolateRgb : d3v3_interpolateString : b instanceof d3v3_Color ? d3v3_interpolateRgb : t === "object" ? Array.isArray(b) ? d3v3_interpolateArray : d3v3_interpolateObject : d3v3_interpolateNumber)(a, b);
  } ];
  ...
  function d3v3_scale_linearTickFormat(domain, m, format) {
    var precision = -Math.floor(Math.log(d3v3_scale_linearTickRange(domain, m)[2]) / Math.LN10 + .01);
    return d3v3.format(format ? format.replace(d3v3_format_re, function(a, b, c, d, e, f, g, h, i, j) {
      return [ b, c, d, e, f, g, h, i || "." + (precision - (j === "%") * 2), j ].join("");
    }) : ",." + precision + "f");
  }
  d3v3.scale.log = function() {
    return d3v3_scale_log(d3v3.scale.linear().domain([ 0, Math.LN10 ]), 10, d3v3_scale_logp, d3v3_scale_powp, [ 1, 10 ]);
  };
  function d3v3_scale_log(linear, base, log, pow, domain) {
    function scale(x) {
      return linear(log(x));
    }
    ...
    scale.domain = function(x) {
      if (!arguments.length) return domain;
      if (x[0] < 0) log = d3v3_scale_logn, pow = d3v3_scale_pown; else log = d3v3_scale_logp, 
      pow = d3v3_scale_powp;
    ...
    scale.nice = function() {
      linear.domain(d3v3_scale_nice(domain, nice).map(log));
      return scale;
    };
    ...
    scale.tickFormat = function(n, format) {
      ...
      var b = Math.log(base), k = Math.max(.1, n / scale.ticks().length), f = log === d3v3_scale_logn ? (e = -1e-12, 
    ...
    scale.copy = function() {
      return d3v3_scale_log(linear.copy(), base, log, pow, domain);
    };
    function nice() {
      return log === d3v3_scale_logp ? {
        floor: floor,
        ceil: ceil
      } : {
        floor: function(x) {
  ...
  function d3v3_scale_powp(x) {
    return Math.exp(x);
  }
  function d3v3_scale_logn(x) {
    return -Math.log(x > 0 ? 0 : -x);
  }
  function d3v3_scale_pown(x) {
    return -Math.exp(-x);
  }
  ...
  function d3v3_scale_pow(linear, exponent, domain) {
    ...
    scale.nice = function() {
      return scale.domain(d3v3_scale_nice(domain, d3v3_scale_linearNice));
    };
  ...
  d3v3.time.format.iso = Date.prototype.toISOString && +new Date("2000-01-01T00:00:00.000Z") ? d3v3_time_formatIsoNative : d3v3_time_formatIso;
  ...
  function d3v3_time_scale(linear, methods, format) {
    ...
    scale.ticks = function(m, k) {
      var extent = d3v3_scaleExtent(scale.domain());
